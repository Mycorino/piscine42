...
void    doormaster(int hight, int wide, int shift, int size)
{
    ...
    while( i < hight)
    {
        ...
        while(j < wide-2)
        {
            if (((wide)-1 -(size-(size -1)%2))/2 -i +size +1 <= j && j < wide - ((wide)-1 -(size-(size -1)%2))/2 -i +size - 1 && size - i + 1 < size - (size-1)%2)
            {
                if (i == 3 - size % 2 + (size - (size -1)%2)/2 && j == wide - ((wide)-1 -(size-(size -1)%2))/2 -i +size - 3 && size >= 5)
                    ft_putchar('$');
                else
                    ft_putchar('|');
            }
